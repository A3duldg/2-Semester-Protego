...
public class EmployeeDB implements EmployeeDBIF {
	...
	// remove: private static final ConcurrentHashMap<Integer, Object> shiftLocks = new ConcurrentHashMap<>(); 

	private static final String CONNECT_SHIFT_TO_EMPLOYEE_Q = "INSERT INTO EmployeeShift (employeeId, shiftId) VALUES (?, ?)";

	private static final String GET_ALL_EMPLOYEES_Q = "SELECT e.employeeId, p.firstName, p.lastName, p.phone, p.email, a.address, a.city, a.postalNr FROM Employee e JOIN Person p ON e.employeeId = p.personId JOIN AddressCityPostal a ON p.addressId = a.addressId";
	...
	public ArrayList<Employee> getAllEmployees() throws DataAccessException {
		...
		try (Connection con = db.getConnection();
				PreparedStatement stmt = con.prepareStatement(GET_ALL_EMPLOYEES_Q);
	...
	public void connectShiftToEmployee(Employee employee, Shift shift) throws DataAccessException {
		...
		synchronized (lock) {
			...
			try {
				...
				try (PreparedStatement countShiftStmt = con.prepareStatement(COUNT_EMPLOYEES_FOR_SHIFT_Q)) {
